...
const Queue = () => {
  ...
  const [systemStatus, setSystemStatus] = useState({ status: 'online', color: '#22c55e', text: 'System Online' });
  const [isPaused, setIsPaused] = useState(false);
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [newVehicle, setNewVehicle] = useState({ name: '', type: '', id: '', fuel: 0, eta: 0 });
  ...
  const openAddModal = () => {
    setNewVehicle({ name: '', type: '', id: '', fuel: 0, eta: 0 });
    setIsModalOpen(true);
  };
  ...
  const addNewVehicle = () => {
    ...
    setQueue(prev => [
      ...prev,
      { ...newVehicle, status: "Scheduled", fuel: parseFloat(newVehicle.fuel) }
    ]);
    ...
    const interval = setInterval(() => {
      setQueue(prev => {
        ...
        if (inProgressIndex !== -1 && newQueue[inProgressIndex].eta === 0) {
          ...
          if (nextWaitingIndex !== -1) newQueue[nextWaitingIndex].status = "In Progress";
        }
        if (inProgressIndex === -1 || newQueue[inProgressIndex].eta === 0) {
          ...
          if (waitingCount < 3) {
            ...
            if (nextScheduledIndex !== -1) newQueue[nextScheduledIndex].status = "Waiting";
          }
        }
        return newQueue;
      });
    ...
    <div className="bg-[rgba(31,_33,_33,_1)] text-gray-100 min-h-dvh pt-45 px-6">
          ...
          <div className="bg-[rgba(38,_40,_40,_1)] rounded-lg p-6 w-full max-w-md">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input type="text" name="name" value={newVehicle.name} onChange={handleInputChange} className="w-full bg-gray-700 rounded px-3 py-2 text-white" placeholder="e.g., H2-Car 1"/>
              </div>
              <div>
                <label className="block text-gray-300 mb-1">Vehicle Type</label>
                <select name="type" value={newVehicle.type} onChange={handleInputChange} className="w-full bg-gray-700 rounded px-3 py-2 text-white">
              ...
              <div>
                ...
                <input type="text" name="id" value={newVehicle.id} onChange={handleInputChange} className="w-full bg-gray-700 rounded px-3 py-2 text-white" placeholder="e.g., DR-011"/>
              </div>
              <div>
                <label className="block text-gray-300 mb-1">Fuel Needed (kg)</label>
                <input type="number" name="fuel" value={newVehicle.fuel} onChange={handleInputChange} className="w-full bg-gray-700 rounded px-3 py-2 text-white" placeholder="Enter fuel in kg" min="0.1" step="0.1"/>
              </div>
              <div className="bg-gray-800 p-3 rounded">
                ...
                <div className="text-gray-400 text-sm mt-1">(Calculated: Fuel / 0.8 kg per minute)</div>
              </div>
            </div>
            <div className="flex justify-end space-x-3 mt-6">
              <button onClick={() => setIsModalOpen(false)} className="px-4 py-2 bg-gray-600 hover:bg-gray-500 rounded-lg font-medium">Cancel</button>
              <button onClick={addNewVehicle} className="px-4 py-2 bg-green-700 hover:bg-green-600 rounded-lg font-medium">Add Vehicle</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-8 gap-4">
        ...
        <div className="flex flex-wrap gap-2">
          <button onClick={openAddModal} className="px-3 py-1.5 bg-gray-700 hover:bg-gray-600 rounded-lg text-sm font-medium transition-colors">Add to Queue</button>
          ...
          <button onClick={removeCompleted} className="px-3 py-1.5 bg-red-700 hover:bg-red-600 rounded-lg text-sm font-medium transition-colors">Remove Completed</button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        {queue.map((item) => (
          <div key={item.id} className={`bg-[rgba(38,_40,_40,_1)] rounded-lg p-4 shadow-lg border-l-4 ${item.status === "Complete" ? 'border-green-500' : item.status === "In Progress" ? 'border-blue-500' : item.status === "Waiting" ? 'border-yellow-500' : 'border-purple-500'} transition-all duration-300 hover:scale-[1.02]`}>
            ...
            <div className="mt-3 space-y-1">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-medium text-white">{item.eta} min{item.eta === 1 ? '' : 's'}</span>
              </div>
            </div>
          </div>
        ))}
      ...
      <div className="mt-8 grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-[rgba(38,_40,_40,_1)] rounded-lg p-4">
